  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (responseData?.errors && Array.isArray(responseData.errors)) {
        ...
        setErrors([
          'We could not reach the registration service. Please check that the backend is running and allows requests from this frontend, then try again.',
        ]);
      } else if (err instanceof Error && err.message) {
        setErrors([err.message]);
      } else {
        setErrors(['Registration failed due to an unexpected error. Please try again.']);
      }
    } finally {
      setIsLoading(false);
    }
