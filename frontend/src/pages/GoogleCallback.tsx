      ...
      try {
        ...
        if (error && typeof error === 'object') {
          ...
          errorMessage = axiosLikeError.response?.data?.message ||
            axiosLikeError.response?.data?.errors?.[0] ||
            errorMessage;

          if (!axiosLikeError.response && (axiosLikeError.code === 'ERR_NETWORK' || axiosLikeError.request)) {
            errorMessage = 'Google sign-in succeeded, but the app could not finish loading your account. Please check the backend connection or local CORS settings and try again.';
          } else if (!axiosLikeError.response?.data && axiosLikeError.message) {
