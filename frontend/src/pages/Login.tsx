...
import { BookOpen, Lock, User, Loader2, ArrowRight, AlertCircle, Mail, CheckCircle } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (responseData?.errorCode === 'EMAIL_NOT_VERIFIED') {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setErrors(['Failed to resend verification email. Please check the email address and try again.']);
    } finally {
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-gray-50 dark:bg-slate-900 transition-colors duration-200">
        <div className="w-full max-w-md">
          ...
          {emailNotVerified && !resendSuccess && (
            <div className="mb-6 p-5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl animate-fade-in">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                    Check your inbox for a verification link, or enter your email below to resend it.
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-amber-300 dark:border-amber-700 rounded-lg text-gray-800 dark:text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-400"
                />
                <button
                  type="button"
                  onClick={handleResendVerification}
                  disabled={isResending || !resendEmail.trim()}
                  className="px-4 py-2 text-sm font-medium bg-amber-600 hover:bg-amber-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5"
                >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  type="text"
                  value={username}
                  onChange={(e) => { setUsername(e.target.value); setErrors([]); setEmailNotVerified(false); }}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  type="password"
                  value={password}
                  onChange={(e) => { setPassword(e.target.value); setErrors([]); setEmailNotVerified(false); }}
                  className="input pl-12"
