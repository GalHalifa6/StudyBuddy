...
import SessionEndedScreen from './session-room/SessionEndedScreen';
import {
  Video,
  MicOff,
  MessageCircle,
...
const SessionRoom: React.FC = () => {
  ...
  const normalizeSessionFileUrl = useCallback((fileUrl?: string) => {
    if (!fileUrl) {
      return undefined;
    }

    return fileUrl.startsWith('http') ? fileUrl : resolveApiUrl(fileUrl);
  }, []);

  const toChatMessage = useCallback((message: SessionMessage): ChatMessage => ({
    ...
    fileUrl: normalizeSessionFileUrl(message.fileUrl),
    fileName: message.fileName,
    language: message.language,
  }), [normalizeSessionFileUrl]);

  const syncSharedFile = useCallback((message: SessionMessage) => {
    if (message.type !== 'file' || !message.fileName || !message.fileUrl) {
      return;
    }

    const normalizedFileUrl = normalizeSessionFileUrl(message.fileUrl);
    if (!normalizedFileUrl) {
      return;
    }

    setFiles((prev) => {
      if (prev.some((file) => file.id === message.id)) {
        return prev;
      }
  ...
  const handleWebSocketMessage = useCallback((message: SessionMessage) => {
    ...
    const newMsg = toChatMessage(message);
    console.log('>>> Adding message to chat from:', newMsg.sender);
    setMessages((prev) => [...prev, newMsg]);
    syncSharedFile(message);
  }, [syncSharedFile, toChatMessage]);
  ...
  const loadSession = useCallback(async () => {
    ...
    try {
      const [sessionData, persistedMessages] = await Promise.all([
        studentExpertService.getSession(numericSessionId),
        sessionService.getSessionMessages(numericSessionId).catch((error) => {
      ...
      try {
        const participantsData = await sessionService.getSessionParticipants(numericSessionId);
      ...
      const historicalMessages = persistedMessages.map((message) => toChatMessage(message));
      const historicalFiles = persistedMessages
        .filter((message) => message.type === 'file' && message.fileName && message.fileUrl)
        .map((message) => ({
          id: message.id,
          name: message.fileName!,
          url: normalizeSessionFileUrl(message.fileUrl!)!,
          uploadedBy: message.senderName,
          uploadedAt: new Date(message.timestamp),
          size: 'Shared in chat',
        }));
      ...
      setFiles([]);
      setMessages([]);
      setSessionLoadError('We could not load the latest session details.');
    } finally {
      setLoading(false);
    }
  }, [normalizeSessionFileUrl, sessionId, toChatMessage]);
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      showError('Failed to start the session. Please try again.');
    }
  };
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      showError('Failed to end the session. Please try again.');
    }
  };
  ...
  const handleSendMessage = () => {
    ...
    if (!isConnected) {
      showError('Live chat is disconnected. Reconnect before sending a message.');
      return;
    }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!isConnected) {
      showError('Reconnect before sharing files with the session.');
      return;
    }
  ...
  const handleShareCode = () => {
    if (!isConnected) {
      showError('Reconnect before sharing code with the session.');
      return;
    }
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900/20 to-gray-900 flex items-center justify-center p-4">
        <div className="text-center bg-gray-800/50 backdrop-blur-lg rounded-2xl p-8 border border-gray-700 max-w-lg w-full">
          <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-white mb-2">Session unavailable</h2>
          <p className="text-gray-400 mb-6">
            {sessionLoadError || "This session may have been cancelled or doesn't exist."}
          </p>
  ...
  if (showSessionEndedScreen) {
    return (
      <SessionEndedScreen
        ...
        messageCount={messages.filter(m => m.type === "text").length}
  ...
  return (
    <div className={`h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-gray-900 flex flex-col ${isFullscreen ? 'fixed inset-0 z-50' : ''}`} style={{ height: '100vh', maxHeight: '100vh', overflow: 'hidden' }}>
      ...
      {(!isConnected || sessionStatus === 'waiting') && (
        <div className="px-4 pt-4">
          <div
            className={`rounded-2xl border px-4 py-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between ${
              !isConnected
          ...
          >
            <div className="flex items-start gap-3">
              <AlertCircle className={`w-5 h-5 mt-0.5 shrink-0 ${!isConnected ? 'text-red-300' : 'text-amber-300'}`} />
              <div>
                <p className="font-medium">
                  {!isConnected ? 'Live sync is offline' : 'Session waiting room'}
                </p>
                <p className={`text-sm ${!isConnected ? 'text-red-100/80' : 'text-amber-100/80'}`}>
                  {!isConnected
                    ? 'Chat, file sharing, and whiteboard updates will send again once the connection recovers.'
                    : isSessionExpert
                      ? 'Participants can join and chat now. Start the session when you are ready.'
                      : 'You can stay in the room and use chat while you wait for the expert to begin.'}
                </p>
              </div>
            </div>
            {!isConnected && (
              <button
                onClick={() => loadSession()}
                className="self-start rounded-xl border border-red-400/30 px-3 py-2 text-sm font-medium text-red-100 hover:bg-red-500/10 transition"
              >
                Retry connection
              </button>
            )}
              ...
              <>
                ...
                <div className="p-4 border-t border-gray-700/50">
                  <div className="flex gap-2">
                    ...
                    />
                    <button onClick={handleSendMessage} disabled={!newMessage.trim() || !isConnected} className="p-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <div className="space-y-2">
                  ...
                  {files.length === 0 && (
                    <div className="text-center py-12 px-4">
                      <div className="w-14 h-14 rounded-2xl bg-gray-700/40 border border-gray-700/60 text-purple-300 flex items-center justify-center mx-auto mb-4">
                        <FileText className="w-6 h-6" />
                      </div>
                      <p className="text-white font-medium mb-2">No files shared yet</p>
                      <p className="text-sm text-gray-400 max-w-xs mx-auto">
                        Share worksheets, notes, or starter files here so everyone stays aligned during the session.
                      </p>
                    </div>
                  )}
                </div>
