...
public class StudyGroup {
    ...
    @JsonIgnoreProperties({"groups", "createdGroups", "courses", "messages", "files", "password", "profileEmbedding"})
    private Set<com.studybuddy.user.model.User> members = new HashSet<>();

    @OneToMany(mappedBy = "group", cascade = CascadeType.ALL)
    @JsonIgnoreProperties({"group", "hibernateLazyInitializer", "handler"})
    private Set<com.studybuddy.messaging.model.Message> messages = new HashSet<>();

    @OneToMany(mappedBy = "group", cascade = CascadeType.ALL)
    @JsonIgnoreProperties({"group", "hibernateLazyInitializer", "handler"})
    private Set<com.studybuddy.file.model.FileUpload> files = new HashSet<>();

    @OneToMany(mappedBy = "group", cascade = CascadeType.ALL)
    @JsonIgnoreProperties({"group"})
    private Set<com.studybuddy.group.model.RoomShare> roomShares = new HashSet<>();
}
